...
$$
\Delta P_i = P_i - P_{\text{base},i}.
$$

Do đó:

$$
P_i = P_{\text{base},i} + \Delta P_i.
$$
...
$$
\Delta P_i = \beta_0 + \beta_1 R_i + \beta_2 H_i + \varepsilon_i.
$$
...
$$
\mathbb{E}\left[\Delta P_i \mid R_i = 0, H_i = 0\right] = \beta_0,
$$

$$
\mathbb{E}\left[\Delta P_i \mid R_i = 1, H_i = 0\right] = \beta_0 + \beta_1,
$$

$$
\mathbb{E}\left[\Delta P_i \mid R_i = 0, H_i = 1\right] = \beta_0 + \beta_2,
$$

và:

$$
\mathbb{E}\left[\Delta P_i \mid R_i = 1, H_i = 1\right] = \beta_0 + \beta_1 + \beta_2.
$$
...
$$
d(i) \sim \operatorname{Categorical}\left(\frac{1}{K}, \ldots, \frac{1}{K}\right).
$$

Mỗi tài xế có một driver effect riêng:

$$
\alpha_d \sim \mathcal{N}\left(0, \sigma_{\text{driver}}^2\right).
$$

Ví dụ:

$$
\sigma_{\text{driver}} = 2{,}000.
$$
...
$$
O_i = \left(x_i^O, y_i^O\right).
$$

Điểm đích của chuyến xe thứ $i$ là:

$$
G_i = \left(x_i^G, y_i^G\right).
$$

Các tọa độ có thể được sinh theo phân phối đều:

$$
x_i^O, y_i^O, x_i^G, y_i^G \sim \operatorname{Uniform}(0, 10).
$$

Khoảng cách Euclidean được tính bằng:

$$
D_i = \sqrt{\left(x_i^G - x_i^O\right)^2 + \left(y_i^G - y_i^O\right)^2}.
$$
...
$$
\gamma_j^O, \qquad j = 1, \ldots, J.
$$

Mỗi khu vực điểm đích có một hiệu ứng riêng:

$$
\gamma_j^G, \qquad j = 1, \ldots, J.
$$
...
$$
\operatorname{Hour}_i \in \{0, 1, \ldots, 23\}.
$$
...
$$
\operatorname{Hour}_i \in \{7, 8, 9, 16, 17, 18\}.
$$
...
$$
R_i \sim \operatorname{Bernoulli}(p_{\text{rain}}).
$$
...
$$
T_i = 3 + 2.5D_i + 8H_i + 3R_i + \eta_i.
$$

Trong đó:

$$
\eta_i \sim \mathcal{N}\left(0, 2^2\right).
$$

Để tránh thời gian âm hoặc không hợp lệ, sử dụng:

$$
T_i = \max\left(T_i, 1\right).
$$
...
$$
P_{\text{base},i} = 10{,}000 + 6{,}000D_i + 500T_i.
$$
...
$$
\Delta P_i = \beta_0 + \beta_1R_i + \beta_2H_i + \alpha_{d(i)} + \gamma_{O_i}^O + \gamma_{G_i}^G + \varepsilon_i.
$$
...
$$
\alpha_d \sim \mathcal{N}\left(0, 2{,}000^2\right).
$$

Origin-zone effect được sinh từ:

$$
\gamma_j^O \sim \mathcal{N}\left(0, 1{,}500^2\right).
$$

Destination-zone effect được sinh từ:

$$
\gamma_j^G \sim \mathcal{N}\left(0, 1{,}000^2\right).
$$

Sai số ngẫu nhiên được sinh từ:

$$
\varepsilon_i \sim \mathcal{N}\left(0, 3{,}000^2\right).
$$
...
$$
P_i = \max\left(P_i, P_{\text{base},i}\right).
$$
...
$$
\mathcal{M}_0: \qquad \Delta P_i = \beta_0 + \beta_1R_i + \beta_2H_i + \varepsilon_i.
$$
...
$$
\widehat{\beta}_0 \approx 2{,}000,
$$

$$
\widehat{\beta}_1 \approx 8{,}000,
$$

và:

$$
\widehat{\beta}_2 \approx 12{,}000.
$$
...
$$
\mathcal{M}_1: \qquad \Delta P_i = \beta_0 + \beta_1R_i + \beta_2H_i + \gamma_{O_i}^O + \gamma_{G_i}^G + \varepsilon_i.
$$
...
$$
\mathcal{M}_2: \qquad
\Delta P_i = \beta_0 + \beta_1R_i + \beta_2H_i + \gamma_{O_i}^O + \gamma_{G_i}^G +
\sum_{d=2}^{K}\alpha_d\mathbb{1}\left\{d(i) = d\right\} + \varepsilon_i.
$$
...
$$
\Delta P_i = \beta_0 + \beta_1R_i + \beta_2H_i + \gamma_{O_i}^O + \gamma_{G_i}^G + \alpha_{d(i)} + \varepsilon_i.
$$
...
$$
\beta_{0,d} = \beta_0 + \alpha_d.
$$

Nếu muốn tác động của mưa và giờ cao điểm khác nhau giữa các tài xế, sử dụng random-slopes model:

$$
\Delta P_i = \beta_0 + \alpha_{d(i)} + \left(\beta_1 + u_{1,d(i)}\right)R_i + \left(\beta_2 + u_{2,d(i)}\right)H_i + \gamma_{O_i}^O + \gamma_{G_i}^G + \varepsilon_i.
$$

Tác động của mưa đối với tài xế $d$ là:

$$
\beta_{1,d} = \beta_1 + u_{1,d}.
$$

Tác động của giờ cao điểm đối với tài xế $d$ là:

$$
\beta_{2,d} = \beta_2 + u_{2,d}.
$$
...
$$
H_0: \gamma_j^O = 0 \quad \text{với mọi } j,
$$
...
$$
H_0: \gamma_j^G = 0 \quad \text{với mọi } j,
$$
...
$$
\mathcal{D}_{\text{train}} = \left\{i: t_i \leq t_{0.8}\right\},
$$

và:

$$
\mathcal{D}_{\text{test}} = \left\{i: t_i > t_{0.8}\right\}.
$$
...
$$
\operatorname{MAE} = \frac{1}{n_{\text{test}}}\sum_{i \in \mathcal{D}_{\text{test}}}\left|\Delta P_i - \widehat{\Delta P}_i\right|.
$$

Root Mean Squared Error được tính bằng:

$$
\operatorname{RMSE} = \sqrt{\frac{1}{n_{\text{test}}}\sum_{i \in \mathcal{D}_{\text{test}}}\left(\Delta P_i - \widehat{\Delta P}_i\right)^2}.
$$

Hệ số xác định được tính bằng:

$$
R^2 = 1 - \frac{\sum_{i=1}^{n}\left(\Delta P_i - \widehat{\Delta P}_i\right)^2}{\sum_{i=1}^{n}\left(\Delta P_i - \overline{\Delta P}\right)^2}.
$$

Nên so sánh các mô hình:

$$
\mathcal{M}_0, \qquad \mathcal{M}_1, \qquad \mathcal{M}_2.
$$
...
$$
\min_{\boldsymbol{\beta}} \sum_{i=1}^{n}\left(\Delta P_i - \widehat{\Delta P}_i\right)^2.
$$

Để biểu diễn bằng một đường liên tục, có thể bổ sung một feature liên tục, chẳng hạn khoảng cách $D_i$, vào mô hình minh họa:

$$
\Delta P_i = \beta_0 + \beta_1R_i + \beta_2H_i + \beta_3D_i + \varepsilon_i.
$$
...
$$
\widehat{\Delta P}_i = \widehat{\beta}_0 + \widehat{\beta}_3D_i.
$$
...
$$
\Delta P_i \quad \text{và} \quad \widehat{\Delta P}_i.
$$
...
$$
\widehat{\mu}_{00} = \widehat{\beta}_0,
$$

$$
\widehat{\mu}_{10} = \widehat{\beta}_0 + \widehat{\beta}_1,
$$

$$
\widehat{\mu}_{01} = \widehat{\beta}_0 + \widehat{\beta}_2,
$$

và:

$$
\widehat{\mu}_{11} = \widehat{\beta}_0 + \widehat{\beta}_1 + \widehat{\beta}_2.
$$
...
$$
\widehat{\beta}_0 \approx \beta_0,
$$

$$
\widehat{\beta}_1 \approx \beta_1,
$$

và:

$$
\widehat{\beta}_2 \approx \beta_2.
$$
...
$$
\operatorname{RMSE}_{\mathcal{M}_2} < \operatorname{RMSE}_{\mathcal{M}_1} < \operatorname{RMSE}_{\mathcal{M}_0}.
$$
...
$$
\Delta P_i = \beta_0 + \beta_1R_i + \beta_2H_i + \varepsilon_i
$$
